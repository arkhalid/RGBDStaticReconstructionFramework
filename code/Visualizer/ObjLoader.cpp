...
bool ObjLoader::loadOBJ(const char* dataPath, const char * objFileName, Mesh* mesh)
{
	...
	while (1)
	{
		...
		if (strcmp(lineHeader, "newmtl") == 0)
		{
			...
			while (strcmp(lineHeader, "map_Kd") != 0)
			{
				//printf("Hearder %s\n", lineHeader);
				fscanf(fileMtl, "%s", lineHeader);
			}
			...
			/*if(strcmp(lineHeader, "Kd") == 0)
				fscanf(fileMtl, "%f %f %f", &color.r,&color.g,&color.b);*/
			Material m;
			strcpy(m.tag, tag);
			if(textureFlag)
				strcpy(m.map_Kd, map);
	...
	for (unsigned int i = 0; i< mesh->numGroups; i++)
	{
		...
		for (unsigned int j = 0; j < vertexIndices[i].size(); j++)
		{
			...
			mesh->unGroupedMesh.vertices.push_back(vertex);
			mesh->unGroupedMesh.uvs.push_back(uv);
			mesh->unGroupedMesh.normals.push_back(normal);
			mesh->unGroupedMesh.indices.push_back(count++);
		}
	}
	mesh->unGroupedMesh.numTri = count / 3;

	return true;
}

bool ObjLoader::loadWeightedOBJ(const char * dataPath, const char * objFileName, UnGroupedMesh & mesh,  bool rgb)
{
	char objPath[256];
	sprintf(objPath, "%s%s", dataPath, objFileName);
	...
	while (1)
	{
		...
		if (strcmp(lineHeader, "usemtl") == 0)
		{
			...
			if(rgb == false)
				fscanf(file, " Material%d\n", &materialId);
			else
			{
				fscanf(file, "%s", lineHeader);
				if (strcmp(lineHeader, "blue") == 0)
					materialId = 3;
		...
		{
			mesh.FaceType.push_back(materialId);
		}
	}
	fclose(file);
	return false;
